 ...
 */
aware.extractHtmlContent = function (element) {
 ...
 * @returns text
 */
aware.cleanText = function(object) {
 ...
 */
aware.isDisplayed = function (element) {
 ...
 */
aware.sendKeys = function(element, str, clear, keypress) {
 ...
 */
aware.waitForDomChanged = function (targetElement, fun, timeout, config, firstExecute) {
 ...
 */
aware.waitLoaded = async function() {
    await aware.waitForDomChanged(document.body, () => document.readyState == 'complete', 5000, {}, true)
    await aware.sleep(500)
}

/**
 * Wait for the element to present
 */
aware.waitForElementPresent = function (targetElement, cssSelector, timeout) {
    targetElement = targetElement || document.body
    return aware.waitForDomChanged(targetElement, () => targetElement.querySelector(cssSelector), timeout, { attributes: false }, true)
}

/**
 * Wait for the element to displayed
 */
aware.waitForElementDisplayed = function (targetElement, cssSelector, timeout) {
    targetElement = targetElement || document.body
    return aware.waitForDomChanged(targetElement, () => {
 ...
 * Wait for the element to present
 */
aware.waitForElementNotPresent = function (targetElement, cssSelector, timeout) {
    targetElement = targetElement || document.body
    return aware.waitForDomChanged(targetElement, () => !targetElement.querySelector(cssSelector), timeout, { attributes: false }, true)
}
 ...
 */
aware.waitForElementNotDisplayed = function (targetElement, cssSelector, timeout) {
    targetElement = targetElement || document.body
    return aware.waitForDomChanged(targetElement, () => {
        let element = targetElement.querySelector(cssSelector)
        if (element) {
